...
| `auth.path` | string | `$XDG_STATE_HOME/ccgate/<target>/auth_key.<provider>.json` | (`file` only) Path to a local regular file. Absolute, `~/`-prefixed, and relative paths are all accepted; relative paths resolve from the hook's *working directory at fire time*, **not** the directory of the config file (so `path: 'key.json'` written in `~/.claude/ccgate.jsonnet` resolves to `<project_root>/key.json`, not `~/.claude/key.json`). Omit the field to use the default path under `$XDG_STATE_HOME/ccgate/<target>/` — an external rotator that writes there works with no extra config. |
