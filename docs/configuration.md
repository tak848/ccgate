...
| Block: `provider` (`name` / `model` / `base_url` / `auth` / `timeout_ms` / `reasoning_effort`) | A layer that writes `provider` **replaces the entire block**. | Embedded `provider: {name: anthropic, model: claude-haiku-4-5}` + global `provider: {name: openai, model: gpt-4o-mini}` → final `provider: {name: openai, model: gpt-4o-mini}`. |
...
| `provider.reasoning_effort` | string                         | `"none"`                                                                      | How much the model reasons before answering. `"none"` / `"minimal"` / `"low"` / `"medium"` / `"high"` / `"xhigh"` / `"max"`, or `""` to send nothing. Accepted values are model-specific; see [docs/providers.md](providers.md#reasoning_effort). |
