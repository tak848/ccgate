...
`"command": "ccgate"` (subcommand なし、legacy 呼び出し) も永続的に動作します -- bare `ccgate` が Claude Code hook の正規呼び出しです。新規セットアップでは `"ccgate claude"` の明示形を推奨 (Claude / Codex の両 hook を同じ dotfiles に書くとき意図が読みやすい)。
...
引数なしで stdin から読み込む `ccgate` は `ccgate claude` と完全に等価。永続的なデフォルトであり、既存の `"command": "ccgate"` 設定は v0.6 multi-target リファクタ後も無変更で動作します。
...
| `settings_permissions`              | 背景 hint として転送                                | 等価物なし。`~/.codex/config.toml` 取り込みは follow-up                                       |
