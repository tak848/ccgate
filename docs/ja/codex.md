...
- **Experimental**: Codex hooks は upstream で experimental 扱い (2026-04 時点)。スキーマや挙動が予告なく変更される可能性あり。OpenAI の [Codex hooks docs](https://developers.openai.com/codex/hooks) を一次情報として参照し、特定 field に依存する前に再検証してください
- **Linux/macOS で検証済、Windows は未検証**: OpenAI docs には `windows_managed_dir` が一級 config field として記載されているため、binary レベルでは block されません。ccgate の Codex flow は Windows で動作未検証 -- Windows で使う場合は untested 扱い
...
- `allow`: 読み取り専用 Bash 検査、project script による build/test、repo 内パッケージインストール、feature branch git 操作、ユーザーが明示的に trust している MCP server で read-only `tool_input` の MCP tool
...
- `environment`: heterogeneous tool surface、trusted-repo 境界、path scope ルール、`apply_patch` は write surface (auto-allow しない)、`recent_transcript` は不在

`apply_patch` は意図的に `allow` に**含めません**: PermissionRequest はまさに Codex がユーザーに prompt しようとする手前で発火するので、ここで auto-allow すると LLM がユーザーの最終防衛線をバイパスできてしまうため。Override したい場合は project-local rule で specific な path subtree に scope を絞った allow を追加してください (リスクを受容できる場合のみ)。
...
- Codex が `permission_mode = plan` を expose した時点での検出 ([plan L#1])
- `transcript_path` JSONL の解析と `recent_transcript` field の追加 ([plan L#2])
- `~/.codex/config.toml` `prefix_rules` 取り込み -- `forbidden` -> deny / `prompt` -> fallthrough ([plan L#3])
- `approval_policy` / `sandbox_mode` を system prompt context として取り込み ([plan L#4])
- fork-only schema field を runtime payload で検証 ([plan L#5])

これらは follow-up GitHub issue で追跡。v0.6 PR は意図的に「multi-target plumbing + tool-agnostic Codex hook」までに scope を絞ってます。
