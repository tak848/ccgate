...
| `provider.reasoning_effort` | string                         | `"none"`                                                                        | 回答前にモデルにどれだけ reasoning させるか。`"none"` / `"minimal"` / `"low"` / `"medium"` / `"high"` / `"xhigh"` / `"max"`、または `""` で何も送らない。ccgate は検証せず、受理される値は接続先次第。[docs/ja/providers.md](providers.md#reasoning_effort) 参照。 |
