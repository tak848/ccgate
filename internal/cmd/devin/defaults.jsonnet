// ccgate defaults for the Devin CLI PermissionRequest hook.
//
...
// Fall through to Devin's own approval prompt when uncertain (set
// fallthrough_strategy=allow|deny in your overrides for fully unattended
// runs -- at your own risk).
//
// Devin hooks fire for exec, edit, write, apply_patch, MCP tool calls, and any
// other surface Devin exposes via PermissionRequest. The rules below are
// written tool-agnostically and reference exec command shapes only as
...
//   - ~/.config/devin/ccgate.jsonnet (global), or
//   - <repo>/.devin/ccgate.local.jsonnet (project-local, untracked-only)
...

{
  ...
  // it reads <main_worktree>/.devin/ccgate.local.jsonnet (untracked
  ...
  allow: [
    'Read-only operations: exec inspection commands (ls, cat, head, tail, less, file, stat, find/grep without -exec/--delete, git status/log/diff/show/branch/remote -v), or any tool whose tool_input shape implies pure read (no writes, no network calls with side effects).',
  ...
  environment: [
    'Tool surface: Devin hooks fire for exec, edit, write, apply_patch, MCP tool calls (mcp__<server>__<tool>), and other tool kinds. Classify by tool_name + tool_input shape rather than assuming a single surface.',
    ...
    'Devin hooks fire only when Devin would otherwise stop and ask the user, which is the prompt the user installed ccgate to skip. Returning fallthrough sends them that prompt anyway, so reserve fallthrough for cases that are genuinely ambiguous (suspect intent, malformed payload, mismatch between description and tool_input, or a tool surface ccgate has no rule for). Default to allow / deny when guidance clearly applies.',
    'Devin HookInput does not carry a recent_transcript field. Decide from tool_name + tool_input + cwd; if intent is ambiguous, return fallthrough (do NOT invent transcript context).',
  ],
}
