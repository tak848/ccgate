...
func writePlanModeRules(b *strings.Builder) {
	...
	b.WriteString("  OK shapes: Claude Code Read/Glob/Grep; MCP read-shaped tools (e.g. `mcp__*__search_*`, `mcp__*__list_*`, `mcp__*__get_*`, `mcp__*__read_*`); pure read-only shell commands (`cat`, `ls`, `find`, `rg`, `grep`, `jq`, `sort`, `head`, `tail`, `wc`) and pipelines composed solely of such commands; editing the active plan file. Specific third-party CLIs (gh, docker, kubectl, etc.) and specific git subcommands are NOT listed here — whether those are allowed in plan mode is the user's policy decision and belongs in allow guidance.\n")
