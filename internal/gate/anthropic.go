...
func writePlanModeRules(b *strings.Builder) {
	...
	b.WriteString("Deny guidance below still applies. Evaluate it first: if a deny guidance rule matches, return deny (or fallthrough if recent_transcript shows the user explicitly requested the exact operation). Deny guidance can block even read-only operations — e.g. reads from sibling worktrees or out-of-repo paths.\n")
	b.WriteString("Otherwise classify:\n")
	b.WriteString("- allow: The operation is (a) side-effect-free (purely read-only / query), OR (b) an edit to the active plan file that Claude Code's plan-mode workflow designated. Implementation-side operations (build, test, package install, git commits/push, edits to project source / production / config / binaries) are NEVER allowed in plan mode — allow guidance below does NOT override this boundary. For compound commands, every subcommand separated by | && || ; |& & or newline MUST independently satisfy (a) or (b). Absence from allow guidance is NOT a reason to fallthrough in plan mode; allow guidance is not required.\n")
	b.WriteString("  OK shapes: Claude Code Read/Glob/Grep; MCP read-shaped tools (e.g. `mcp__*__search_*`, `mcp__*__list_*`, `mcp__*__get_*`, `mcp__*__read_*`); pure read-only shell commands (`cat`, `ls`, `find`, `rg`, `grep`, `jq`, `sort`, `head`, `tail`, `wc`) and pipelines composed solely of such commands; editing the active plan file. Third-party CLIs (gh, docker, kubectl, etc.) and git subcommands are not listed as categorical OK shapes because safety depends on the specific invocation — a specific invocation still passes (a) when it is demonstrably side-effect-free (e.g. `git status`, `git log`, `git diff`). Allow guidance does NOT grant an additional exception beyond (a)/(b); side-effecting variants of these CLIs are covered by the deny rule regardless.\n")
	b.WriteString("  NOT OK shapes: `curl ... | sh`; `jq ... | xargs rm`; any pipeline containing a writing or installing subcommand; shell invocations of untrusted downloads.\n")
