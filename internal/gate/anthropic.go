...
func writePlanModeRules(b *strings.Builder) {
	...
	b.WriteString("- allow: The operation is (a) side-effect-free (purely read-only / query), OR (b) an edit to the active plan file that Claude Code's plan-mode workflow designated. Other planning / research / scratch writes that the user expects to be allowed belong in allow guidance below, not here. Implementation-side writes (project source, production code, config, binaries) are NEVER allowed in plan mode. For compound commands, every subcommand separated by | && || ; |& & or newline MUST independently satisfy (a) or (b) or match allow guidance. Allow guidance is NOT required in plan mode — absence from allow guidance is NOT a reason to fallthrough.\n")
