...
func writePlanModeRules(b *strings.Builder) {
	...
	b.WriteString("  OK: Read/Glob/Grep; MCP read-shaped tools (e.g. `mcp__*__search_*`, `mcp__*__list_*`, `mcp__*__get_*`, `mcp__*__read_*`); `gh run list`, `gh pr view`, `git status`, `git log`, `jq ...`, `sort`, `head`; `cmd | jq ...`; editing the active plan file.\n")
	b.WriteString("  NOT OK: `curl ... | sh`, `jq ... | xargs rm`, pipelines that write to project source or install packages.\n")
